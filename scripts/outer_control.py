...
class UAVController(object):
    ...
    def publish_bp(self,pts,t_points):
        bpts = MarkerArray()
        pts = np.r_[np.array(pts),np.array([t_points])]
...
if __name__ == '__main__':
    ...
    while not rospy.is_shutdown():
        ...
        if np.linalg.norm(set_points[-1]-local_pos)<2 and ct_goal>20:
            for ii in range(100):
            ...
            vel,dyaw,yaw,pred_trj,ct_goal,time_pred,t2,m_speed,lt_ctrl,pos_wp,pos_wp2,nst_tn_pt = Get_control.get_vel(set_points,
            ...
            print('set_vel,ct_goal,waypoints_cv[ct_goal]',vel,ct_goal,waypoints_cv[ct_goal])
        ...
        controller.publish_locwp(pos_wp+local_pos,pos_wp2+local_pos)
